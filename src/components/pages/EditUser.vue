<template>
  <div>
    ...
    <button type="button" :disabled="!canEditUser" @click="editUser">Edit User</button>
...
export default {
  ...
  methods: {
    getCurrentUser() {
      const url = `${process.env.VUE_APP_API_URL}api/user/get-user/${this.userId}`;
    ...
    checkUserData(userData) {
      ...
      this.newUserEmail = userData.email;
      this.newUserUsername = userData.username;
      this.newUserEmail = userData.email;
      this.newUserType = userData.userType;
      // Double bang to coax into a boolean
      this.newUserEnabled = !!userData.enabled;

      if ("firstName" in userData) {
        this.newUserFirstName = userData.firstName;
      }
      if ("lastName" in userData) {
        this.newUserLastName = userData.lastName;
      }
